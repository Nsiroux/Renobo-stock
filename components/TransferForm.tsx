...
export default function TransferForm({ variants, locations }: Props) {
  ...
  async function submitTransfer(formData: FormData) {
    ...
    if (!response.ok) {
      setError(result?.error ?? 'Er liep iets mis bij het registreren van de transfer.')
      return
    }

    setSuccess('Transfer succesvol geregistreerd.')
    setQuantity('')
  ...
  return (
    <div className="rounded-3xl bg-white p-5 shadow-sm">
      <h2 className="text-xl font-semibold text-neutral-900">Transfer registreren</h2>
      <p className="mt-2 text-sm text-neutral-600">
        Verplaats stock tussen locaties.
      </p>
      ...
      >
        ...
        <div>
          <label className="mb-2 block text-sm font-medium text-neutral-700">Naar locatie</label>
          <select
            name="to_location_id"
            value={toLocationId}
        ...
        >
          {isPending ? 'Bezig...' : 'Transfer registreren'}
        </button>
      </form>
    </div>
  )
